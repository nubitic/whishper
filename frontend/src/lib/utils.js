...
export let CLIENT_API_HOST = browser ? `${env.PUBLIC_API_HOST}` : `${env.PUBLIC_INTERNAL_API_HOST}`;
export let CLIENT_WS_HOST = browser ? `${env.PUBLIC_API_HOST.replace("http://", "").replace("https://", "")}` :  `${dev ? env.PUBLIC_INTERNAL_API_HOST.replace("http://", "").replace("https://", "") : ""}`;
